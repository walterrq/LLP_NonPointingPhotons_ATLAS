...
def calculate_delta_r(df_photons, df_leptons):
    """
    Calculates the ΔR between the most energetic photon and electron in each event.
    ...
    Returns:
    delta_r_values (list): A list of ΔR values for each event.
    """
    delta_r_values = []
    ...
    for event in events:
        ...
        if event in df_photons.index.get_level_values('N') and event in df_leptons.index.get_level_values('N'):
            # Extract the most energetic photon (id = 0)
            photon = df_photons.loc[(event, 0)]
            # Extract the most energetic electron (id = 0)
            electron = df_leptons.loc[(event, 0)]
...
def plot_delta_r_histogram(delta_r_values, destiny):
    """
    ...
    plt.title('Histogram of ΔR between Most Energetic Photon and Electron')
    plt.xlabel('ΔR')
    plt.ylabel('Frequency')
    
    # Save the histogram as a PNG file
    plt.savefig(f"{destiny}/delta_r_histogram.png")
    
    # Optionally, display the plot
    plt.show()

#todo TERMINAMOS HISTOGRAMA --------------------------------------------

# Example usage:
# Assuming your DataFrames for photons and leptons are named 'photons' and 'leptons'
destiny = "/path/to/save/histograms"  # Replace with your actual directory

# Calculate ΔR values
delta_r_values = calculate_delta_r(photons, leptons)

# Plot ΔR histogram
plot_delta_r_histogram(delta_r_values, destiny)
