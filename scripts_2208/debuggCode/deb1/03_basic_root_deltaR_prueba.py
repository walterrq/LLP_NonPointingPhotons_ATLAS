...
origin = f"/Collider/scripts_2208/data/clean/deltaR_change/"
...
for typex in types[:]:
    for tevx in tevs[:]:
        for order in orders[:]:
            destiny = f"/Collider/scripts_2208/data/clean/deltaR_change/{order}_pickle/"
            for file_inx in sorted(glob.glob(origin + f"*{typex}*{tevx}_{order}.root"))[:]:
