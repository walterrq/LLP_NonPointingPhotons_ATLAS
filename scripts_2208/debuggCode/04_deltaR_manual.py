...
def isolate_photons(df_photons, df_leptons, delta_r_max=0.2, pt_min=0.1, pt_ratio_max=0.065):
    ...
    """
    df_isolated_photons = pd.DataFrame(columns=['N', 'eta', 'phi', 'E'])
    ...
    for event in events:
        ...
        if event in df_photons.index.get_level_values('N') and event in df_leptons.index.get_level_values('N'):
            ...
            if any(isolated_photon_mask):
                ...
                df_isolated_photons = pd.concat([df_isolated_photons, isolated_photons[['N', 'eta', 'phi', 'E']]])
    ...
    df_isolated_photons = df_isolated_photons.sort_values(by=['N', 'E'])
    # Set 'N' and 'id' as a multi-index
    df_isolated_photons_multi = df_isolated_photons.set_index(['N', 'E'])
