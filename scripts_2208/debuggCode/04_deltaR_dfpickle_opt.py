...
def plot_delta_r_histogram(delta_r_values, alpha, destiny, output_name):
    ...
    plt.savefig(f"{destiny}/deltaR_op_{output_name}_{alpha}.png")
...
for alpha in [4, 5, 6]:
    ...
    for type in ['ZH', 'WH', 'TTH']:
        ...
        destiny = f"./data/deltaR_isol_op/{type}_{alpha}/"
        Path(destiny).mkdir(exist_ok=True, parents=True)
        ...
        efphotons = isolate_photons_opt(efphotons, eftrack)
        ...
        np.savetxt(f"{destiny}/deltaR_Alpha_op{alpha}_{type}.txt", deltaR_ph_track)
        ...
        # Plot ΔR histogram
        np.savetxt(f"{destiny}/deltaR_Alpha_op{alpha}_{type}_ecals.txt", deltaR_ph_ecals)
        # Plot ΔR histogram
