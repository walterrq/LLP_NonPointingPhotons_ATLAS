...
'''
The main utility of this code resides in generating the detector simulation with Delphes after
the run in different deltaR min conditions for photon isolations.
Once the deltaR optimal was found, this code can be ignored and one can work with the simpler version:
03_basic_root.py
'''

#This script extracts data from the ROOT files generated by Delphes.
#In the end, we will have 3 dataframes: one for leptons, one for jets, and one for photons.
