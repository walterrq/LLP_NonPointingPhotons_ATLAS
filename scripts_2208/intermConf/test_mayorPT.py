    ...
    for sentence in hepmc:     
        ...
        if len(line) > 0:
            ...
            if line[0] == 'E':
                ...
                if (len(pt_matrix)!=0):
                    max_pt = np.argmax(pt_matrix[:,0])#seleccionamos col con max pt
                    max_ptrow = pt_matrix[max_pt,1:] #anadimos elementos sin pt como termino
                    result_string = ' '.join(map(str, max_ptrow)) #separamos por espacio
                    f.write(result_string + '\n')

                #! definition of momentum matrix of 7 elements
                pt_matrix = np.empty((0, 8)) 
                ...
                if(es_foton_final):
                    ...
                    if (realizar_analisis_cumple): #Si es que la particula es foton y es particula final (si no es negativo)    
                        ...
                        try:                                                         
                            ...
                            # multiplicamos por d_scalar el cual cambia las unidades a cm
                            ...
                            R1, R2 = eta_functions_R_abs.eta_func_R_abs(eta1,eta2)
                            ...
                            #!Comenzamos el print en txt

                            #format for output string
                            '''
                            linetxt = f"{R1} {R2} {eta1} {eta2} {zsimpl_value} {abs(c_z[-1])} {z_atlas_value}\n"

                            f.write(linetxt)
                            #sys.exit("Salimos")
                            '''
                            #!append row to M matrix
                            linetxt = [pt ,R1, R2, eta1, eta2, zsimpl_value, abs(c_z[-1]), z_atlas_value]
                            pt_matrix = np.vstack([pt_matrix, linetxt])
